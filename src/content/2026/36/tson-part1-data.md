...
**Status:** Working revision. The 2026 revision series is subject to change without compatibility guarantees. When finalised, this specification will be published as **TSON version 1** and frozen (§1.2, principle 6); until then, revisions are released under the 2026 series. This revision restates base type resolution's applicability over what types a position rather than over the header (§4.1), adds `!boolean` to the built-in vocabulary (§5.5), widens the fifth reporting outcome from a refusal to *not judged* so that an unobtainable schema is not a verdict (§8.1), replaces §2.9's serialisation sentence with the rule [TSON-SCHEMA] §5.2 now states, qualifies key identity for compound keys (§2.6), and is published beside the first revision of [TSON-JSON], the JSON encoding, which defines the `TSON-Schema` transport field this document points at (§7.1). The lexer is unchanged. Revision 35 removed the claim that TSON is a JSON superset, and with it the rules that existed only for that claim: `null` is no longer a base value (§4), the `\/` escape and the surrogate-pair rules are gone and a braced `\u{…}` escape is added (§7.2.2), a field name is an identifier at every layer (§2.5, §7.7), and a comma may follow a value (§2.4). It restates base type resolution as applying only in schemaless documents (§4.1), replaces the four spelled binary annotations with one `!bytes` (§5.3), splits the temporal `!duration` into `!duration` and `!period` (§5.4), makes the name-hygiene policy and the resource limits reportable properties of a processor rather than of a refusal (§8.1, §8.2, §9.1), and says why the content-hash pin rides in the URI query (§2.2.1). This is the first revision to change the lexer since the Class 1 freeze was declared (§1.3); it does so under principle 6, and the freeze holds from this revision.
...
- **Part 3: JSON Encoding** [TSON-JSON] — the JSON encoding of the same model: the JSON reader this document names in §6, its encoding rules stated against [TSON-SCHEMA] §7's shape, and the `TSON-Schema` transport field.
...
Whether an absent value at a record field is written as `_` or the field omitted is a serialisation choice **where the field's declaration gives both spellings the same meaning, and the declaration decides everywhere else** ([TSON-SCHEMA] §5.2, §7.6): a field may be declared so that its key must be written (an absent value is then `_`), so that `_` is refused (an absent value is then omission), or so that omission injects a default while `_` clears it. A schemaless document has no declaration, and both spellings read as absence there; a schema-directed encoder writes the spelling the declaration requires, and one without the schema omits, which is right wherever the two mean the same. Implementations SHOULD provide a mechanism for controlling this at the encoder level.
...
Base type resolution applies to a token that carries no built-in type annotation, **at a position nothing types** — a document whose header carries no `!!schema` (§2.2), being read into no declared type, as when a processor produces a document-shaped value. A built-in annotation overrides base resolution for its token (§5). Where the position *is* typed — by a schema, by a tag, or by a **declared host type a processor is reading into** that names a family §5 defines — the type's own parsing contract reads the token and this section does not apply: under a schema every value is typed by its position or by its tag, each declared atom type owns its own parsing contract, and there is no third way to read a token ([TSON-SCHEMA] §4.2, §7.1). The host-typed case is the same rule seen from the reader's side rather than the document's: a processor binding `{ i: "12" }` to a 32-bit integer field reads `12`, as a schema declaring `int32` there would, and reports a refusal in the family's own words rather than a bind failure naming a host type the sender has never heard of; the division is over whether anything types the position, not over what the header says. A target that names no family the series defines — an opaque host object, a type outside §5's vocabulary — leaves the position untyped, and base resolution applies. The tokens `true` and `false` have special status only under base type resolution (§4.2). Resolution applies to **data values only**: field names are names (§2.5), and map keys are text, never resolved — in `{ "007": 007 }` the key is the name `007` and the value is a string by fallthrough (leading zeros fail the `integer` production, §4.3), and `07` and `7` are distinct map keys because key identity is textual at this layer (§2.6).

A Class 1 value is one of three things — an untyped token, a token carrying a built-in annotation from §5's vocabulary, or a container — and this section governs only the first. The vocabulary is the schemaless way to say what a token *means*: `!date 2025-03-13`, `!uuid 9f1c…`, `!int32 007` are typed and checked with no schema in scope. Base resolution is what a token gets when its author chose not to say and nothing else says for them.
...
At a position nothing types (§4.1), the parser MUST attempt resolution in this order:
...
| Annotation | Format | Host value |
|------------|--------|------------|
| `!boolean` | the tokens `true` and `false`, case-sensitive | boolean |
...
`!boolean` is core's `boolean`, the enum `[true false]` ([TSON-SCHEMA] §7.4): it accepts exactly the two tokens and refuses any other as the enum-member violation it is — a validation error, as every enum's member set gives. A typed position does not consult the form, so `!boolean "true"` and `!boolean true` are one value; §4.2's special status for the unquoted tokens is a base-resolution rule that a typed position never reaches. The row exists for the same reason `!text` does: base resolution already recovers a boolean from an unquoted `true`, and the annotation asserts the case where it is in doubt — a quoted `"true"` at a position an author means as boolean — and names the family a host-typed read (§4.1) reads by. Its absence from earlier revisions was an oversight, not a design.
...
The **identifier profile** governs the decoded text of a name — a field name, type name, annotation name, parameter name, or the member of an enum whose profile is `IDENTIFIER` ([TSON-SCHEMA] §7.4; a `TEXT` enum's members are values) — however that text was spelled, and is the subject of the identifier grammar (§7.7):
...
TSON documents use the media type `application/tson` (intended for IANA registration). Version information is not encoded in the media type; if disambiguation is needed in HTTP contexts, implementations MAY use `application/tson; version=1`. The schema that governs a body is named in band by `!!schema` (§2.2) and, over HTTP, out of band by the **`TSON-Schema`** header field, defined in [TSON-JSON] §3.5 for every TSON-carrying body: an RFC 9651 structured field holding a schema reference as a String Item, a projection of the document's binding rather than an alternative to it, so that intermediaries can route by schema without opening the message and a body in an encoding with no directive syntax has a channel at all. Where the field and `!!schema` are both present they MUST agree by canonical identity (§2.2.1) — scheme and pin excluded — and disagreement is an error, never a precedence question, on the same posture as §2.2.1's two-digest rule; the field states the sender's claim and is no licence to validate an unmarked document against a schema of the receiver's choosing. File extensions carry the same distinction the media-type parameter does. The unversioned extension **`.tn`** makes no stability claim: it is the extension of the 2026 revision series — this document's own bundled schemas use it — and remains appropriate for any document published without a frozen-version guarantee. The extension **`.tn1`** is a positive claim of TSON version 1 stability and is reserved for the version 1 freeze: it MUST NOT be used before that release, and future major versions use correspondingly numbered extensions (`.tn2`, …). Renaming a document from `.tn` to `.tn1` at the freeze changes its identifying URI and therefore its canonical identity (§2.2.1); references pinned during the revision series do not carry over and are re-pinned against the frozen identities. Whether a TSON file is a data document or a schema document is determined by its header (§2.2), not its extension: classification requires at most two directives of lookahead and no value parsing, so streams, previews, and content sniffers can classify a document from its opening bytes.
...
**Not judged is a fifth outcome, not a verdict.** Two kinds of outcome leave a document without a verdict, and share the property that makes them one outcome rather than a category: the same document may be well-formed, valid, and accepted in full by the next processor along. A **refusal** means *this processor declined* — under its stated policy, data version and limits: the name-hygiene mechanisms of §8.2 and the resource limits of §9.1 — and never that the document is wrong; a conforming processor may legitimately not refuse at all. An **unavailable schema** means *this processor could not obtain what it would judge against* — a `!!schema`, `!!meta` or `!!import` reference its library does not hold and its policy or its network did not supply ([TSON-SCHEMA] §10.1, §11.2) — and says nothing about whether the document conforms, since nothing has read the schema; a pin that *was* obtained and does not match is a resolver error, a finding about bytes the document's reference commits to ([TSON-SCHEMA] §10.2). Both MUST be distinguishable from the four categories above, and each MUST name what produced it — the rule or limit that refused, or the reason the schema could not be obtained — since the three hygiene mechanisms, each limit, and each fetch outcome want different remedies and that is what a consumer routes on. Both are reported in the *same* report as the four categories, a refusal at the rule's position and an unavailable schema at the reference: a consumer reads a report to repair a document, and a repair channel split in two is repaired in two passes, so nothing here asks for either to travel apart from the errors it arrives with. What the report carries about the policy is stated in §8.2 and §9.1. The distinction is what lets two conforming processors legitimately disagree on whether they judged a document while never disagreeing on validity.
...
- **They are not validity.** Each depends on Unicode data that the Unicode Consortium declines to freeze — `confusables.txt`, `IdentifierStatus.txt`, and the script-based restriction levels of UTS #39 — so a verdict can change under a routine Unicode Character Database refresh. A content-addressed document (§2.2.1) must mean the same thing forever, so nothing here may decide whether a document is valid. A document that fails a check is **refused by this processor**, reported as one member of §8.1's fifth outcome.
...
| RFC 9651 | Structured Field Values for HTTP (the `TSON-Schema` field, [TSON-JSON] §3.5; §7.1) | https://www.rfc-editor.org/rfc/rfc9651 |
