...
**Resolved-output consumers.** A consumer that ingests only resolved schema values (§8, §10.1) — never schema source — is fully conforming with no support for templates or parameters (§5.10). An open entry appears in output as a `type_definition` whose `body` is a `!template` — the kernel constructor whose instances *are* held bodies: the parameter names, and the application as text (§5.10, §8.1); a consumer of closed entries never meets one, since every entry a data document's type can reach is closed by the closed-entry rule (§5.10), so the wire never presents template machinery to a data-path consumer, and a consumer that does meet one needs the schema parser §8.1's ingest already requires of it.
...
| Record template `<T> { ... }` | `source: record`; not instantiable; `body: !template { parameters: [T]  template: "!record { fields: [...] }" }` (§5.10) | `container` |
...
| meta-kernel.tn | TSON Meta-Kernel (companion artifact) | https://tson.io/2026/36/m/meta-kernel.tn?sha256=28e4497bb5bd9c3e628547c0a9de9e6802a5b848959df96e4812a43fc53c307d |
| meta.tn | TSON Meta-Schema (companion artifact) | https://tson.io/2026/36/m/meta.tn?sha256=bf967ed0e3e2cd1d56864bc06c2c9fbc69270a60c978dd81c1db83c172d3b00e |
| core.tn | TSON Core Type Library (companion artifact) | https://tson.io/2026/36/m/core.tn?sha256=3953b2a6b6fc6d254d013c80d9247b73b02068014df47f8858afb2db1a3c14e7 |
