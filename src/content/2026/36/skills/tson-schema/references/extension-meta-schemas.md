...
```
!!id:"https://example.com/2026/36/meta-http-1.tn"
!!meta:"https://tson.io/2026/36/m/meta-kernel.tn"
!!import:"https://tson.io/2026/36/m/meta.tn"
```

`!!meta` is the **kernel**, not `meta.tn`: only a schema whose own `!!meta` names the meta-kernel may declare an
entry that IS-A `top` — that is, a constructor (§2.2.2) — so an extension is a *sibling* of `meta.tn`, and it imports `meta.tn` for the vocabulary its own
...
- **Mixing operands is free.** A base kind seeds the kind and plain records lend vocabulary, exactly as the
  kernel's `uri_type => text_type & atom_specification & { … }` does. This is the pattern for shared
...
`type_name` and not `text`. `type_ref` is the slot that participates in reference resolution and
...
!!meta:"https://example.com/2026/36/meta-http-1.tn"
...
|---|---|---|
| `!!meta:"…/m/meta.tn"` with a constructor declaration | an entry that IS-A `top` needs the meta-kernel as governing target | `!!meta:"…/m/meta-kernel.tn"` + `!!import:"…/m/meta.tn"` |
...
In the Java implementation an instance of a `data`-kinded constructor is *built* by a bound class — a public record
