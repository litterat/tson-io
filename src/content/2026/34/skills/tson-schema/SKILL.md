...
> Revision note: identifiers such as `https://tson.io/2026/34/m/meta.tn` carry the revision number. A new revision changes every such URL and every pin. Never invent a `?sha256=` digest — compute it with the tson-data skill's `pin.py`, copy one from a real file, or leave the reference unpinned.
