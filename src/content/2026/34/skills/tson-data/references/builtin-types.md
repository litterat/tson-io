...
Source: TSON Part 1 §5 and §7.6 (2026 Revision 34). These annotations apply **only in schemaless processing**. Under a `!!schema`, every `!name` resolves through the schema's namespace; the core type library (`https://tson.io/2026/34/m/core.tn`) declares types with the same names and the same parsing contracts, so a schema that imports core gives its data documents this vocabulary back.
