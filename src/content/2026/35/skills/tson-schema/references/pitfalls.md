...
| `~array & { … }` in a user schema | the `~` marker was removed in Revision 35; a constructor is an entry that IS-A `top`, declarable only under the meta-kernel | refine or apply instead (or write a meta-schema — `extension-meta-schemas.md`) |
