...
| Source | TSON |
|---|---|
| `type: object`, `properties`, `required` | `{ … }`; unlisted-in-`required` → `?`; `additionalProperties: true` → a map field (mark it `@rest` if an encoding should flatten into it) or `dynamic` (records are always closed) |
